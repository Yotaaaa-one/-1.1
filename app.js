...
function resolveFullShot(current,opt,timing,rng=Math.random){
  ...
  if(reachedGreenArea&&rng()<Math.max(.08,Math.min(.98,.72+accuracyBoost+approachBoost))){current.lie="green";const basePutt={PERFECT:1.4,GOOD:3,NORMAL:6,MISS:10,BAD:15}[timing.grade]*(attacking?.72:layingUp?1.35:1);current.distance=+(Math.max(.5,basePutt*(.65+rng()*.7)).toFixed(1));current.position={progress:.98,lateral:direction.value*.12};current.greenPosition=greenPointForDistance(current.distance,direction.side);return{finished:false,penalty:false,outcome:"green",direction:direction.side,title:"グリーンオン",urbanHazard:h.greenLabel,text:`${h.greenLabel}へオン。ピンまで ${current.distance}m`,shotNumber}}
...
async function renderCharacterShotAnimation({character,shotType,timingResult,isPutt,missDirection}){
  ...
  const speed=animationSpeedMultiplier(),labelText=timingResult?.label||"SHOT",typeLabel=isPutt?"パット":shotType==="attack"?"攻めショット":shotType==="layup"?"刻みショット":"通常ショット",fallbackDuration=Math.round((isPutt?640:timingResult?.grade==="PERFECT"?820:760)*speed);
