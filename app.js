...
const holes=URBAN_HOLE_DATA.map((hole,index)=>({number:index+1,...hole}));
...
function migrate(saved){
  ...
  const character=characterById(saved.characterId||saved.selectedCharacterId);saved.version=6;saved.roundHoles=roundModeByHoles(saved.roundHoles).holes;saved.finalResult ||= null;saved.courseTheme ||= COURSE_THEME;saved.characterId ||= character.id;saved.selectedCharacterId ||= saved.characterId;saved.characterName ||= character.name;saved.control ??= saved.accuracy||character.control;saved.shortGame ??= character.shortGame;saved.putting ??= character.putting;saved.aim ||= "center";saved.lastShotOrigin ||= null;
...
function renderCourse(h){
  const onGreen=state.lie==="green",features=h.features||[],show=(id,feature)=>$(id)?.classList.toggle("hidden",!features.includes(feature));$("#courseVisual").dataset.tone=h.tone||"day";$("#holeName").textContent=`${h.number}H · ${h.name}`;$("#holeComment").textContent=h.comment;$("#courseMap").classList.toggle("hidden",onGreen);$("#greenView").classList.toggle("hidden",!onGreen);$("#courseVisual").classList.toggle("green-active",onGreen);
  show("#urbanRoad","road");show("#urbanCrosswalk","crosswalk");show("#urbanBuildings","buildings");show("#urbanPark","park");show("#urbanRooftop","rooftop");show("#urbanRail","rail");show("#urbanNeon","neon");show("#urbanFountain","fountain");
