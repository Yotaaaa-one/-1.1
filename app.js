...
function finalDirectionFor(current,timing,h,opt={}){const aim=aimById(current.aim),timingSide=timing.side==="left"?-1:timing.side==="right"?1:0,performance=characterPerformance(current.characterId);const lieSpread=current.lie==="bunker"?1.45:current.lie==="rough"?1.17:current.lie==="fairway"?.86:1;const gradeSpread={PERFECT:.015,GOOD:.07,NORMAL:.15,MISS:.31,BAD:.54}[timing.grade]*(normalizeShotId(opt.id)==="attack"?1.18:1)*performance.sideMissMultiplier;const precision=Math.max(.55,1-(current.accuracy-60)*.009-(current.control-60)*.005);const windDirection=current.hole%2?-1:1;const windDrift=windDirection*(current.winds[current.hole]-1)*(h.windFactor||1)*.018;const value=Math.max(-1.15,Math.min(1.15,aim.value+timingSide*gradeSpread*lieSpread*precision+windDrift));return{aim,value,side:value<-.14?"left":value>.14?"right":"center",error:Math.abs(value-aim.value)}}
...
function roundStats(roundHoles,source=state){
  const scores=source.scores.slice(0,roundHoles),logs=source.shots.slice(0,roundHoles).flatMap((holeShots,holeIndex)=>holeShots.filter(shot=>shot.type==="shot").map(shot=>({...shot,choice:displayShotChoice(shot.choice),hole:holeIndex+1}))),fullShots=logs.filter(log=>log.choice!=="パットする"),putts=logs.filter(log=>log.choice==="パットする"),strongShots=logs.filter(log=>isStrongChoice(log.choice)),parOn=fullShots.filter(log=>log.outcome==="green").length;
...
globalThis.PGAEngine={holes,ROUND_MODES,CHARACTERS,AIM_OPTIONS,TIMING_PROFILES,SETTINGS_KEY,DEFAULT_GAME_SETTINGS,FIELD_SHOT_FALLBACK,characterById,characterPerformance,normalizeShotId,displayShotChoice,isStrongChoice,normalizeShotOption,aimById,roundModeByHoles,timingProfileFor,timingResult,freshState,migrate,optionsFor,safeShotOptionsFor,renderShotCards,ensureShotCardsVisible,canUseAttack,consumeAttackShot,puttDifficultyLabel,lieMultiplierFor,finalDirectionFor,approachTravelFor,shotInChanceFor,shotInResultFor,snapshotShotOrigin,restoreShotOrigin,resolveFullShot,resolvePutt,updatePosition,mapPoint,greenPointForDistance,quadraticPoint,createShotAnimation,renderCharacterShotAnimation,setShotLock,rankingForRound,roundStats,bestAndTrouble,buildFinalResult,resultComment,normalizeGameSettings,loadGameSettings,animationSpeedMultiplier};
